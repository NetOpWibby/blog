  ...
  const UserAgent = function () {
    ...
    this.getBrowserVersion = function (string) {
      ...
      switch (this.Agent.browser) {
        case "Chrome":
          if (this._BrowsersVersions.Chrome.test(string)) return RegExp.$1;
          break;

        case "Chromium":
          if (this._BrowsersVersions.Chromium.test(string)) return RegExp.$1;
          break;

        case "Edge":
          if (this._BrowsersVersions.Edge.test(string)) return RegExp.$1;
          break;

        case "Firefox":
          if (this._BrowsersVersions.Firefox.test(string)) return RegExp.$1;
          break;

        case "IE":
          if (this._BrowsersVersions.IE.test(string)) return RegExp.$2 ? RegExp.$2 : RegExp.$1;
          break;

        case "Opera":
          if (this._BrowsersVersions.Opera.test(string)) return RegExp.$1 ? RegExp.$1: RegExp.$2;
          break;

        case "Safari":
          if (this._BrowsersVersions.Safari.test(string)) return RegExp.$1;
          break;
    ...
    this.getOSVersion = function (string) {
      ...
      switch (true) {
        case this.Agent.isAndroid:
          if (this._OSVersions.Android.test(string)) return RegExp.$1;
          break;

        case this.Agent.isiOS:
          if (this._OSVersions.iOS.test(string)) return (RegExp.$1).replace(/_/g, ".");
          break;

        case this.Agent.isMac:
          if (this._OSVersions.Mac.test(string)) return (RegExp.$1).replace(/_/g, ".");
          break;

        case this.Agent.isWindowsPhone:
          if (this._OSVersions.WindowsPhone.test(string)) return RegExp.$1;
          break;

        case this.Agent.isWindows:
          if (this._OSVersions.Windows.test(string)) return RegExp.$1;
          break;
    ...
    this.testBot = function testBot() {
      ...
      if (isBot) ua.Agent.isBot = isBot[1];
      else if (!ua.Agent.isAuthoritative) ua.Agent.isBot = /bot/i.test(ua.Agent.source);
    };

    this.testWebkit = function testWebkit() {
      const ua = this;

      if (ua.Agent.browser === null && /applewebkit/i.test(ua.Agent.source)) {
    ...
    this.testTablet = function testTablet() {
      ...
      if (/tablet/i.test(ua.Agent.source)) ua.Agent.isTablet = true;
    };
    ...
    this.testCompatibilityMode = function () {
      ...
      if (this.Agent.isIE) {
        if (/Trident\/(\d)\.0/i.test(ua.Agent.source)) {
          ...
          const version = parseInt(ua.Agent.browserversion, 10);
    ...
    this.reset = function reset() {
      const ua = this;
      for (const key in ua.DefaultAgent) ua.Agent[key] = ua.DefaultAgent[key];
